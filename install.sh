...
# install.sh
# -----------------------------------------------------------------------------
# User-level installer for "autotarcompress" CLI tool.
#
...
#       Uninstall autotarcompress from user system
#
#   ./install.sh -h | --help
...
GITHUB_GIT_URL="git+https://github.com/Cyber-Syntax/AutoTarCompress"
readonly GITHUB_GIT_URL

# CLI executable name shown to users
CLI_NAME="autotarcompress"
readonly CLI_NAME
...
find_latest_prerelease_tag() {
  ...
  repo_url="https://github.com/Cyber-Syntax/AutoTarCompress.git"
...
# Remove everything installed by autotarcompress at user level.
#
# This includes:
...
uninstall_autotarcompress() {
  ...
  # 2. Remove install directory (autocomplete + assets)
...
Options:
  ...
  -u, --uninstall           Remove autotarcompress from user system
